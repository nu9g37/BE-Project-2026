...
exports.getCampgrounds = async (req, res, next) => {
  ...
  // Copy req.query
  const regQuery = {...req.query};
  
  // Field to exclude
  const removeFields = ['select', 'sort', 'page', 'limit'];

  // Loop over remove fields and delete them from query
  removeFields.forEach(param => delete regQuery[param]);
  console.log(regQuery);

  // Create query string
  let queryStr = JSON.stringify(regQuery);
  ...
  try {
    ...
    if (!campground) {
      return res.status(404).json({
        success: false,
        msg: `Campground not found with id of ${req.params.id}`
      });
    }
